 ...
 *      return up to 10 results. `signal` cancels a query the rider has typed past.
 */
export async function searchCities(query, options = {}) {
  ...
  const response = await fetchWithTimeout(url, { timeoutMs: 10000, signal: options.signal });
...
export async function reverseGeocode(latitude, longitude) {
  ...
  // Short deadline: a place name is a nicety, and the forecast waits on it.
  const response = await fetchWithTimeout(url, { timeoutMs: 5000, mode: 'cors' });
